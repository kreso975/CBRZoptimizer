...
LRESULT CALLBACK InstructionsWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
   ...
   case WM_CREATE:
   {
      ...
      // Load and show icon
      HICON hIcon = (HICON)LoadImageW(GetModuleHandleW(NULL), MAKEINTRESOURCEW(IDI_ICON1), IMAGE_ICON, 32, 32, 
         LR_DEFAULTCOLOR | LR_SHARED);

      if (hIcon)
      {
         HWND hIconCtrl = CreateWindowW(L"STATIC", NULL, WS_CHILD | WS_VISIBLE | SS_ICON, 20, 20, 32, 32,
            hWnd, NULL, NULL, NULL);
      ...
      hFontTitle = CreateFontW(-16, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                               CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Segoe UI");

      HWND hTitleLabel = CreateWindowW(L"STATIC", L"CBRZoptimizer Instructions", WS_CHILD | WS_VISIBLE,
                                       70, 28, 400, 24, hWnd, NULL, NULL, NULL);

      SendMessageW(hTitleLabel, WM_SETFONT, (WPARAM)hFontTitle, TRUE);

      // Create RichEdit control below header
      hRichEdit = CreateWindowExW(0, L"RICHEDIT50W", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_MULTILINE | ES_READONLY | ES_AUTOVSCROLL | ES_NOHIDESEL, 20, 60, 540, 360, hWnd, (HMENU)IDC_INSTRUCTIONS_TEXT, NULL, NULL);
      ...
      hFontEmoji = CreateFontW( -12, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, 
         CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Segoe UI Emoji");
      ...
      CreateWindowW(L"BUTTON", L"OK", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON, 260, 440, 80, 25, 
         hWnd, (HMENU)IDC_INSTRUCTIONS_OK, NULL, NULL);
      break;
   }
...
void ShowInstructionsWindow(HWND hwndOwner, HINSTANCE hInst)
{
   ...
   HWND hWnd = CreateWindowExW( WS_EX_DLGMODALFRAME, CLASS_NAME, L"Instructions", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU, 
      x, y, width, height, hwndOwner, NULL, hInst, NULL);
